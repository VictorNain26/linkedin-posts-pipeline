...
def test_returns_none_if_no_biases(tmp_data_dir):
    ...
    from config import LEARNINGS_PATH
    LEARNINGS_PATH.parent.mkdir(exist_ok=True)
    LEARNINGS_PATH.write_text(json.dumps({
...
def test_returns_none_if_stale(tmp_data_dir):
    ...
    LEARNINGS_PATH.write_text(json.dumps({
        ...
        "summary": "stale",
        "biases": [{"id": "x", "type": "formula_weight", "key": "y", "instruction": "z", "evidence": "w"}],
    }), encoding="utf-8")
...
def test_returns_block_when_valid(tmp_data_dir):
    ...
    LEARNINGS_PATH.write_text(json.dumps({
        ...
        "biases": [
            {"id": "b1", "type": "formula_weight", "key": "prospect_question",
             "instruction": "privilégier +50%", "evidence": "47 vs 22 impr"},
        ],
    }), encoding="utf-8")
...
def test_caps_at_5_biases(tmp_data_dir):
    ...
    LEARNINGS_PATH.write_text(json.dumps({
        ...
        "summary": "test cap",
        "biases": [
            {"id": f"b{i}", "type": "formula_weight", "key": f"k{i}",
